module.exports = {
   ...
   903: {
      name: 'rpl_authenticationsuccessful',
      type: 'reply'
   },
   // If authentication fails, a 904 or 905 numeric will be sent and the client MAY retry 
   // from the AUTHENTICATE <mechanism> command.
   904: {
      name: 'err_authenticationfailed', // dunno know what may be here
      type: 'error'
   },
   905: {
      name: 'err_authenticationfailed',
      type: 'error'
   },
   // If the client attempts to issue the AUTHENTICATE command after already authenticating
   // successfully, the server MUST reject it with a 907 numeric.
   907: {
      name: 'err_alreadyloggedintoanaccount',
      type: 'error'
   }
};
